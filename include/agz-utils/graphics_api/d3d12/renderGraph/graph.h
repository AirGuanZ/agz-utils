...
inline Vertex *Graph::addVertex(std::string name, int thread, int queue)
{
    return compiler_->addVertex(std::move(name), thread, queue);
}

inline void Graph::addArc(Vertex *head, Vertex *tail)
{
    return compiler_->addArc(head, tail);
}
